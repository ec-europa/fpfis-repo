...
docker exec -u ${UID} ${CONTAINER}  /usr/bin/mock -v -r fpfis-${EL}-x86_64 --spec=/mock/build/SPECS/${SPECFILE} --sources=/mock/build/SOURCES/${PACKAGE} --resultdir=/mock/build/SRPMS --buildsrpm
docker exec -u ${UID} ${CONTAINER} /usr/bin/mock -v --clean -r fpfis-${EL}-x86_64  -D "dist .${DIST}" --resultdir=/mock/build/RPMS --rebuild /mock/build/SRPMS/$(ls -1utr SRPMS/|grep ^${PACKAGE}-.*src\.rpm$|head -1)
